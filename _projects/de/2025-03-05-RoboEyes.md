...
#### Schritt 2: Distanzen mit dem ToF-Sensor messen
...
#### Schritt 3: Richtung der RoboEyes anpassen
...
Achte dabei darauf, dass du diese Codezeilen noch innerhalb der Statusabfrage `if ((!status) && (NewDataReady != 0)) {` platzierst, die Variablen `min_distance` und `min_index` an dieser Stelle ausgelesen werden können. Probier es dann direkt aus!
